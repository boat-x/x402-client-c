...
class ConfigContentGen():
    ...
    # BoAT lite
    def selectliteoption(self):
        bl = self.getEnterAsNoTimed('Do you want to make config for BoAT lite?', 5)
        if bl:
            bl = self.getEnterAsNo("BOAT_LITE_HTTP2?")
            if bl:
                self.config_content += 'BOAT_LITE_HTTP2 = Y\n'
            else:
                print("no http2")
                self.config_content += 'BOAT_LITE_HTTP2 = N\n'

            bl = self.getEnterAsNo("BOAT_LITE_CJSON?")
            if bl:
                self.config_content += 'BOAT_LITE_CJSON = Y\n'
            else:
                print("no BOAT_LITE_CJSON")
                self.config_content += 'BOAT_LITE_CJSON = N\n'

            bl = self.getEnterAsNo("BOAT_LITE_PROTOBUF_C?")
            if bl:
                self.config_content += 'BOAT_LITE_PROTOBUF_C = Y\n'
            else:
                print("no BOAT_LITE_PROTOBUF_C")
                self.config_content += 'BOAT_LITE_PROTOBUF_C = N\n'

            bl = self.getEnterAsNo("BOAT_LITE_PROTOS?")
            if bl:
                self.config_content += 'BOAT_LITE_PROTOS = Y\n'
            else:
                print("no BOAT_LITE_PROTOS")
                self.config_content += 'BOAT_LITE_PROTOS = N\n'


            bl = self.getEnterAsNo("BOAT_LITE_RLP?")
            if bl:
                self.config_content += 'BOAT_LITE_RLP = Y\n'
            else:
                print("no BOAT_LITE_RLP")
                self.config_content += 'BOAT_LITE_RLP = N\n'

            bl = self.getEnterAsNo("BOAT_LITE_UTILITIES?")
            if bl:
                self.config_content += 'BOAT_LITE_UTILITIES = Y\n'
            else:
                print("no BOAT_LITE_UTILITIES")
                self.config_content += 'BOAT_LITE_UTILITIES = N\n'

            bl = self.getEnterAsNo("BOAT_LITE_RPC?")
            if bl:
                self.config_content += 'BOAT_LITE_RPC = Y\n'
            else:
                print("no BOAT_LITE_RPC")
                self.config_content += 'BOAT_LITE_RPC = N\n'
        else:
            self.config_content += 'BOAT_LITE_HTTP2 = Y\n'
            self.config_content += 'BOAT_LITE_CJSON = Y\n'
            self.config_content += 'BOAT_LITE_PROTOBUF_C = Y\n'
    ...
    def gen_export(self):
        ...
        self.config_content += 'export BOAT_LITE_HTTP2\n'
        self.config_content += 'export BOAT_LITE_CJSON\n'
        self.config_content += 'export BOAT_LITE_PROTOBUF_C\n'
        self.config_content += 'export BOAT_LITE_PROTOS\n'
        self.config_content += 'export BOAT_LITE_RLP\n'
        self.config_content += 'export BOAT_LITE_UTILITIES\n'
        self.config_content += 'export BOAT_LITE_RPC\n'
    ...
    def getEnterAsNoTimed(self, content, timeout):
        print("Please input within {} seconds:".format(timeout))
        print(content + '(N/y):');

        rlist, _, _ = select.select([sys.stdin], [], [], timeout)

        if rlist:
            yon = sys.stdin.readline().strip()
            if (yon == '' ):
                print("No\n")
                return False
            elif ((yon == 'y') or (yon == 'Y')):
            ...
            print("Timeout, no input received, set as No")
            return False    
...
def main():
    ...
    if owMakefile:
        ...
        # BoAT lite
        configContent_obj.selectliteoption()
