...
#define BoatLog_hexdump(level, title, buf, len)\
	do{\
		if( level <= BOAT_LOG_LEVEL ){\
			BoatPrintf("%s: "__FILE__":%d, %s(): %s[%d bytes]: ", g_log_level_name_str[level-1], __LINE__, __func__, title, len );\
			if( len > 0 ){\
...
#define BoatLog_hexasciidump(level, title, buf, len)\
	   do{\
		   if( level <= BOAT_LOG_LEVEL ){\
		   BoatPrintf("%s: "__FILE__":%d, %s(): %s[%d bytes]: \n", g_log_level_name_str[level-1], __LINE__, __func__, title, len );\
			   if( len > 0 ){\
